...
def get_tf_terms(w_type):
    ...
    for root, dirs, files in os.walk(DIRECTORY):
        for file in files:
            if file.lower().endswith('.txt') and file.lower().startswith(w_type):
                idx += 1
...
def writing_files(tf_idf_dicts, idf_terms, w_type):
    path_result_begin = f"tf_idf_{w_type}/{w_type}_"
